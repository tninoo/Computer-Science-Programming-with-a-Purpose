...
public class BarChart {
  ...
  public BarChart(String title, String xAxisLabel, String dataSource) {
    ...
    if (xAxisLabel == null) throw new IllegalArgumentException(
      "x-axis label is null"
    );
  ...
  private static Color[] initColors() {
    ...
    // use 20 colors
    // converting a String in the format of "#aec7e8"into a java awt Color
  ...
  private static int getUnits(double xmax) {
    ...
    while (Math.floor(xmax / units) >= 8) {
      ...
      if (units % 9 == 2) units = units * 5 / 2; else units = units * 2;
    }
    return units;
  }
